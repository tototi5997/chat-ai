  ...
  return (
    <Box width="80%" overflow="auto">
      {currentHistory?.content.map((e:contentInterface, i:number) => (
        <Box key={i} width="80%" display={e.origin === 'user' ? '' : 'flex'} backgroundColor={e.origin === 'user' ? '#5E565699' : ''} ml={e.origin === 'user' ? "auto" : ''} mb="20px" borderRadius="8px" p="10px" color="#FDFCFB">
          {e.origin === 'ai' ? <Box position="relative" w="32px" h="42px">
